...
- Produces:
  ...
  - `GET /api/portal/onboarding/agenda/callback?code=&state=` → verifieert state-HMAC + sessie-match, wisselt code, slaat tokens onder `oauth:google:cust:<customerId>`, redirect `/portal/onboarding?order=<id>`.
